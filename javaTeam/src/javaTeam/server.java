...
//최초 접속해서 정보를 받는 부분
	class Connect extends Thread{
		...
		public void run() {
			...
			frame.setText("접속 : 아이디-"+inet.getHostName()+", 주소-"+inet.getHostAddress()+"\n");
	...
	class UserThread extends Thread{
		...
		public void send(String readData) {
			...
			for(int i=0;i<ipVec.size();i++) {
				vo=ipVec.get(i);  
				inet=vo.getInet();
				if(stData[0].equals(inet.getHostAddress())) { //받는사람 ip주소 stdata 보내는사람ㅈ소inet
				try {
